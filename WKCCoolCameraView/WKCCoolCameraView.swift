...
public class WKCCoolCameraView: UIView {

    /// 闪光灯改变的通知
    public let flashChangedNotification: String = "com.flash.changed"
    /// 闪光灯通知info内包含的信息 -> Bool
    public let isFlashOnKey: String = "com.is.flash.on"
    ...
    public weak var delegate: WKCCoolCameraViewDelegate?
    ...
    private var recordEncoder: WKCCoolVideoRecordEncoder?
...
extension WKCCoolCameraView: UIGestureRecognizerDelegate {
    ...
    private func finishRecordVideo() {
        recordEncoder = nil
        if let d = delegate, d.responds(to: #selector(WKCCoolCameraViewDelegate.coolCameraDidVideoRecorded(cool:videoPath:))) {
            d.coolCameraDidVideoRecorded?(cool: self, videoPath: videoPath)
        }
    }
    ...
    private func zoom(scale: CGFloat) {
        do {
            ...
            if let d = delegate, d.responds(to: #selector(WKCCoolCameraViewDelegate.coolCameraDidZoom(cool:zoom:))) {
                d.coolCameraDidZoom?(cool: self, zoom: zoomScale)
            }
        } catch {
    ...
    @objc private func actionGesture(sender: UITapGestureRecognizer) {
        ...
        if let d = delegate, d.responds(to: #selector(WKCCoolCameraViewDelegate.coolCameraDidFocus(cool:point:))) {
            d.coolCameraDidFocus?(cool: self, point: point)
        }
    }
...
extension WKCCoolCameraView: AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate {
   ...
   public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        if let d = delegate, d.responds(to: #selector(WKCCoolCameraViewDelegate.coolCameraDidOutputVideoBuffer(cool:sampleBuffer:))) {
            d.coolCameraDidOutputVideoBuffer?(cool: self, sampleBuffer: sampleBuffer)
        }
        
        if runningMode == .takePhoto {
            runningMode = .common
            let buffer: CVPixelBuffer? = sampleBuffer.imageBuffer
            guard let buf = buffer else { return }
            let image = WKCCoolHelper.image(buffer: buf)
            
            if let d = delegate, d.responds(to: #selector(WKCCoolCameraViewDelegate.coolCameraDidTakePhoto(cool:image:))), let img = image {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
                    if let result = self.cutout(image: img) {
                        d.coolCameraDidTakePhoto?(cool: self, image: result)
                    }
                }
            }
            ...
            if recordEncoder == nil {
                ...
                if frameWidth != 0 && frameHeight != 0 {
                    recordEncoder = WKCCoolVideoRecordEncoder(path: videoPath, height: frameHeight, width: frameWidth, channels: 1, smaples: 44100)
                    return
                }
            }
